...
RUN curl -O https://downloads.apache.org/maven/maven-3/3.3.9/binaries/apache-maven-3.3.9-bin.tar.gz && tar -zxf apache-maven-3.3.9-bin.tar.gz && \
        mv apache-maven-3.3.9 /usr/local/maven && \
        ln -sf /usr/local/maven/bin/mvn /usr/local/bin/mvn && \
        mkdir -p $HOME/.m2 && chmod -R a+rwX $HOME/.m2

ENV GRADLE_VERSION 2.6
RUN curl -sL -O https://services.gradle.org/distributions/gradle-2.6-bin.zip  && \
    unzip gradle-2.6-bin.zip -d /usr/local/ && \
